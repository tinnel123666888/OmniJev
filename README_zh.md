...
**v1.2 开发：** [连续/离散动作头、扩充数据与实测限制](docs/v12_development.md)。第一轮候选未通过闭环发布门槛，目前发布版本仍是 v1.1。
...
一个 4B 的模型，能回答图片、视频、手机与电脑屏幕、游戏和机器人画面中的决策问题。目前发布的演示是离线回放，尚未证明可靠的闭环游戏通关或机器人控制能力。它把画面、你的问题和文本上下文一起读，回答是**校准过的概率**而不是生成的文字，并且可以回答"都不是"。
...
**成绩口径更正：** 下表是有题数上限的任务族汇总，不是完整的逐数据集基准成绩。[查看逐数据集/子集分数与未评测项](docs/dataset_scores_v11.md)。马里奥下一动作准确率只有 43.52%（193 题），56.05% 混合了其他是非题；OK-VQA 93.47% 是包含正确答案的候选选择题准确率，不是官方开放式 VQA 分数；旧 safety 分数只覆盖 HaGRID。
