...
The expanded second bundle contains 44,046 states. A 16,000-step continuation completed on eight A800 GPUs from the first action checkpoint; offline validation selected step **10,000**. All frozen v1.1 checkpoint file hashes remained unchanged. [Per-domain split counts and deduplication manifest](v12_data_manifest_stage2.json). Domain-specific train-only normalization is retained when continuing an existing head; rare discrete actions receive inverse-square-root frequency sampling. Previously inspected offline holdouts are identified as development data, not a new untouched final test.

On its 128-state development subsets, stage two reduced normalized first-command MSE below the constant train-mean baseline for FetchReach (**0.05298 vs 0.26504**), FetchPush (**0.04898 vs 0.19269**) and FetchPickAndPlace (**0.10529 vs 0.40211**). Pygame Mario action accuracy remained only **25.78%**, with balanced accuracy **18.65%**. Stage-one and stage-two sampled subsets differ, so these are not paired before/after improvements. [Every environment and full offline metrics](v12_action_scores.md). Stage-two closed-loop and general-route checks are in progress; this candidate is not approved for release.
